...
const Main = () => {
  ...
  const toggleTodos = useMutation({
    mutationFn: (todo) => axios.patch(`http://localhost:5000/todos/${todo.id}`, { isCompleted: !todo.isCompleted }),
    onSuccess: () => {
      queryClient.invalidateQueries('todos');
  ...
  const editTodos = useMutation({
    mutationFn: ({ id, text }) => axios.patch(`http://localhost:5000/todos/${id}`, { text }),
    onSuccess: () => {
      queryClient.invalidateQueries('todos');
      setOpen(false);
      setEditText('');
  ...
  return (
    <Container maxWidth="md" sx={{ pt: 4 }}>
      <Paper sx={{ p: 4, backgroundColor: theme.palette.background.paper }}>
        ...
        <Box sx={{ overflowY: 'auto', maxHeight: '60vh', mt: 2, scrollbarWidth:'0' }}>
          {getTodos.isLoading ? (
    <Box sx={{ width: '100%' }}>
    <LinearProgress />
  </Box>
            ...
            todos?.map((todo) => (
              ...
              >
                <Checkbox checked={todo.isCompleted} onChange={() => handleComplete(todo)} color="secondary"  />
                <Box sx={{ flexGrow: 1, ml: 2,  }}>
                  <Typography variant="h6" sx={{ textDecoration: todo.isCompleted ? 'line-through' : 'none', color: theme.dark ? 'black' : 'black' }}>
                    {todo.text}
                  </Typography>
                  <Typography variant="body2" sx={{ color: theme.dark ? 'black' : 'black' }}>
                ...
                <Box>
                  <Fab size="small" color="primary" onClick={() => handleClickOpen(todo.id, todo.text)} sx={{ mr: 1 }}>
                    <EditIcon />
                  </Fab>
                  <Fab size="small" color="secondary" onClick={() => handleDelete(todo.id)}>
                    <DeleteIcon />
                  </Fab>
                </Box>
              </Box>
      ...
      <Dialog open={open} onClose={handleClose} PaperProps={{ component: 'form', onSubmit: (e) => { e.preventDefault(); handleEdit(); } }}>
        <DialogTitle>Edit Task</DialogTitle>
        <DialogContent>
          <TextField autoFocus required margin="dense" label="Edit Task" value={editText} onChange={(e) => setEditText(e.target.value)} sx={{width:500}} />
        </DialogContent>
        <DialogActions>
